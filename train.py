...
def train_pipeline(params):
    ...
    if dataset_name == 'cifar10':
        ...
        train_dataset = CustomDataset(root_dir='./train/images', transform=transform)
        train_loader = DataLoader(train_dataset, shuffle=True, batch_size=params['batch_size'], num_workers=params['num_workers'])
        test_dataset = CustomDataset(root_dir='./test/images', transform=transform)
...
def train(args, ratio: float, snr: float):  # deprecated
    ...
    transform = transforms.Compose(
            [transforms.ToTensor(), transforms.Resize((128, 128))])  # the size of paper is 128
    print("loading data of imagenet")
    train_dataset = CustomDataset(root='./train/images', transform=transform)
    train_loader = DataLoader(train_dataset, shuffle=True, batch_size=args.batch_size, num_workers=args.num_workers)
    test_dataset = CustomDataset(root='./test/images', transform=transform)
    test_loader = DataLoader(test_dataset, shuffle=True, batch_size=args.batch_size, num_workers=args.num_workers)
